    ...
    super({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          float response = 0.045 + uActivity * 0.075 + uEvolution * 0.06 + uMutation * 0.095 + uGrowth * 0.07 + uFormChange * 0.18 - uInstability * 0.012;
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          color += uGlowColor * edge * (0.08 + activity * 0.14);
          color += spectral * pow(cells, 6.0) * (0.05 + uMutation * 0.14);
          color *= 0.78 + sin(uTime * 4.0) * (0.04 + activity * 0.05);
          // Keep the chromatic surface below the mobile HDR clipping knee.
          color = color / (1.0 + max(color, vec3(0.0)) * 0.72);
          color = clamp(color, vec3(0.015), vec3(0.88));
          float alpha = clamp(0.78 + edge * 0.12 + activity * 0.06 - uInstability * 0.08, 0.62, 0.9);
